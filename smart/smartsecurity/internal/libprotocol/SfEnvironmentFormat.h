...
#define SF_CREATE_ENVIRONMENT(environment, environmentType) \
    ({ \
    environment* pEnvironment = malloc( sizeof( environment ) ); \
    if ( NULL != pEnvironment ) \
    { \
        memset(pEnvironment, 0x00, sizeof( environment )); \
        pEnvironment->header.size = sizeof ( environment ); \
        pEnvironment->header.type = environmentType; \
    } \
    else \
    { \
        SF_LOG_E( "can not allocate memory for environment;" ); \
    } \
    pEnvironment; \
    })
...
typedef enum
{
    ...
    SF_ENVIRONMENT_TYPE_SND_RCV = 4,        ///< sendmsg()/recvmsg() system call environment
    SF_ENVIRONMENT_TYPE_SCRIPT  = 5,        ///< Script environment
    SF_ENVIRONMENT_TYPE_REPORT  = 6,        ///< Report environment
    SF_ENVIRONMENT_TYPE_MAX
} SF_ENVIRONMENT_TYPE;


/**
****************************************************************************************************
* @typedef SfEnvironmentInfo
* @brief Mapped to blocked environment type's Formart and Execution.
****************************************************************************************************
*/
static const char EnvPro[]    = "PROCESS";
static const char EnvFile[]   = "FILE";
static const char EnvNet[]    = "NETWORK";
static const char EnvMmap[]   = "MMAP";
static const char EnvSnd[]    = "SND_RCV";
static const char EnvScript[] = "SCRIPT";
static const char EnvReport[] = "FILE";
...
static const SfEnvironmentInfo s_EnvironmentInfo[ SF_ENVIRONMENT_TYPE_MAX ] = 
{
    {EnvPro     ,1},  ///< Blocked Process Type
    {EnvFile    ,0},  ///< Blocked File Type
    {EnvNet     ,0},  ///< Blocked Network
    {EnvMmap    ,1},  ///< Blocked mmap type
    {EnvSnd     ,0},  ///< Blcoekd send/recv type
    {EnvScript  ,1},  ///< Blocked script type
    {EnvReport  ,0}   ///< Blocked Report type
};
...
typedef struct __attribute__((__packed__))
{
    ...
    SfExecutionEnvironmentInfo processContext;  ///< Operation execution environment
    /**
     * Pay attention: byte order in the values might be reverted!
     *
     * to convert an addr to string may be used thred unsafe inet_ntoa
     * struct in_addr in;
     * in.s_addr=addr;
     * SLOGD("%s",inet_ntoa(in);
     *
     * port is reverted too
     */
...
/**
****************************************************************************************************
* @struct SfSndRcvEnvironment
* @brief Environment for blocked sendmsg()/recvmsg() calls notification
****************************************************************************************************
*/
typedef struct __attribute__((__packed__))
{
    ...
    SfExecutionEnvironmentInfo processContext;
...
/**
****************************************************************************************************
* @brief                        Destroy SF sendmsg()/recvmsg() environment
* @param [in] pEnv              SF sendmsg()/recvmsg() environment
* @return
****************************************************************************************************
*/
void SfDestroySndRcvEnvironment( SfSndRcvEnvironment* pEnv );
