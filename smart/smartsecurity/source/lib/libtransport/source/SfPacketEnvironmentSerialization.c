                                           ...
                                           SfNetlinkPacket* const pPacket, Int attribute )
{
    ...
    pAttr = nla_nest_start( pPacket->pBuffer, attribute | NLA_F_NESTED);
    if ( pAttr )
    {
...
****************************************************************************************************
*/
static SF_STATUS PutSndRcvEnvironment( const SfProtocolHeader* pHeader, SfNetlinkPacket* pPacket,
                                       Int attribute )
{
    SF_STATUS r = SF_STATUS_FAIL;
    const SfSndRcvEnvironment* pEnv = (const SfSndRcvEnvironment*)pHeader;

    struct nlattr* pAttr = nla_nest_start( pPacket->pBuffer, attribute | NLA_F_NESTED );
    if ( pAttr )
    {
        if ( SF_SUCCESS( SfSerializeExecutionEnvironment( &pEnv->processContext, pPacket,
                                                          SFD_SND_RCV_ENV_EXEC_ENV_ATTR ) )   &&
             !nla_put_u32( pPacket->pBuffer, SFD_SND_RCV_ENV_PROTO_ATTR, pEnv->protocol )     &&
             !nla_put_u32( pPacket->pBuffer, SFD_SND_RCV_ENV_CALL_ATTR, pEnv->type )          &&
             !nla_put_u32( pPacket->pBuffer, SFD_SND_RCV_ENV_SRC_ADDR_ATTR, pEnv->srcAddr )   &&
             !nla_put_u16( pPacket->pBuffer, SFD_SND_RCV_ENV_SRC_PORT_ATTR, pEnv->srcPort )   &&
             !nla_put_u32( pPacket->pBuffer, SFD_SND_RCV_ENV_DEST_ADDR_ATTR, pEnv->destAddr ) &&
             !nla_put_u16( pPacket->pBuffer, SFD_SND_RCV_ENV_DEST_PORT_ATTR, pEnv->destPort ) )
        {
            r = SF_STATUS_OK;
        }
        nla_nest_end( pPacket->pBuffer, pAttr );
...
****************************************************************************************************
*/
static SF_STATUS LoadSndRcvEnvironment( SfSndRcvEnvironment* pEnv, struct nlattr* pAttribute )
{
    SF_STATUS r = SF_STATUS_FAIL;

    struct nlattr* envAttrs [ SFD_SND_RCV_ENV_MAX_ATTR + 1 ] = { 0 };
    if ( !nla_parse_nested( envAttrs, SFD_SND_RCV_ENV_MAX_ATTR, pAttribute, NULL ) )
    {
        struct nlattr *execEnvAttr  = envAttrs[ SFD_SND_RCV_ENV_EXEC_ENV_ATTR ],
                      *protoAttr    = envAttrs[ SFD_SND_RCV_ENV_PROTO_ATTR ],
                      *typeAttr     = envAttrs[ SFD_SND_RCV_ENV_CALL_ATTR ],
                      *srcAddrAttr  = envAttrs[ SFD_SND_RCV_ENV_SRC_ADDR_ATTR ],
                      *srcPortAttr  = envAttrs[ SFD_SND_RCV_ENV_SRC_PORT_ATTR ],
                      *destAddrAttr = envAttrs[ SFD_SND_RCV_ENV_DEST_ADDR_ATTR ],
                      *destPortAttr = envAttrs[ SFD_SND_RCV_ENV_DEST_PORT_ATTR ];
        if ( execEnvAttr && protoAttr && typeAttr && srcAddrAttr && srcPortAttr && destAddrAttr &&
             destPortAttr )
        {
            SfDeserializeExecutionEnvironment( &pEnv->processContext, execEnvAttr );
            pEnv->protocol = nla_get_u32( protoAttr );
            pEnv->type     = nla_get_u32( typeAttr );
            pEnv->srcAddr  = nla_get_u32( srcAddrAttr );
            pEnv->srcPort  = nla_get_u16( srcPortAttr );
            pEnv->destAddr = nla_get_u32( destAddrAttr );
            pEnv->destPort = nla_get_u16( destPortAttr );
            r = SF_STATUS_OK;
        }
    }
    return r;
                                        ...
                                        SfNetlinkPacket* const pNetlinkPacket, Int attribute )
{
    ...
    switch ( pHeader->type )
    {
        ...
        case SF_ENVIRONMENT_TYPE_SND_RCV:
            cb = PutSndRcvEnvironment;
            break;

        default:
            SF_LOG_E( "[unexpected env type(%d)];", pHeader->type );
            break;
    }
                                                 ...
                                                 struct nlattr* envDataAttr )
{
    ...
    switch ( envHeader->type )
    {
        case SF_ENVIRONMENT_TYPE_FILE:
        {
            SfFileEnvironment* pFileEnv = 
                SF_CREATE_ENVIRONMENT( SfFileEnvironment, SF_ENVIRONMENT_TYPE_FILE );
            if (pFileEnv)
            {
                pFileEnv->header = *envHeader;
                LoadFileEnvironment( pFileEnv, envDataAttr );
                pPrtclHdr = (SfProtocolHeader*)pFileEnv;
            }
            break;
        }
        ...
        case SF_ENVIRONMENT_TYPE_PROCESS:
        {
            SfProcessEnvironment* pProcEnv = 
                SF_CREATE_ENVIRONMENT( SfProcessEnvironment, SF_ENVIRONMENT_TYPE_PROCESS );
            if (pProcEnv)
            {
                pProcEnv->header = *envHeader;
                LoadProcessEnvironment( pProcEnv, envDataAttr );
                pPrtclHdr = (SfProtocolHeader*)pProcEnv;
            }
            break;
        }
        ...
        case SF_ENVIRONMENT_TYPE_NETWORK:
        {
            SfNetworkEnvironment* pNetEnv = 
                SF_CREATE_ENVIRONMENT( SfNetworkEnvironment, SF_ENVIRONMENT_TYPE_NETWORK );
            if (pNetEnv)
            {
                pNetEnv->header = *envHeader;
                LoadNetworkEnvironment( pNetEnv, envDataAttr );
                pPrtclHdr = (SfProtocolHeader*)pNetEnv;
            }
            break;
        }
        ...
        case SF_ENVIRONMENT_TYPE_MMAP:
        {
            SfMmapEnvironment* pMmapEnv = 
                SF_CREATE_ENVIRONMENT( SfMmapEnvironment, SF_ENVIRONMENT_TYPE_MMAP );
            if (pMmapEnv)
            {
                pMmapEnv->header = *envHeader;
                LoadMmapEnvironment( pMmapEnv, envDataAttr );
                pPrtclHdr = (SfProtocolHeader*)pMmapEnv;
            }
            break;
        }
        ...
        case SF_ENVIRONMENT_TYPE_SND_RCV:
        {
            SfSndRcvEnvironment* pSndRcvEnv = 
                SF_CREATE_ENVIRONMENT( SfSndRcvEnvironment, SF_ENVIRONMENT_TYPE_SND_RCV );
            if (pSndRcvEnv)
            {
                pSndRcvEnv->header = *envHeader;
                LoadSndRcvEnvironment( pSndRcvEnv, envDataAttr );
                pPrtclHdr = (SfProtocolHeader*)pSndRcvEnv;
            }
            break;
        }
        ...
        default:
        {
            SF_LOG_E( "[unexpected env type(%d)];", envHeader->type );
            break;
        }
    }
    return pPrtclHdr;
